...
def init_add_punc_model(onnx_model_path):
    try:
        session = None
        session = onnxruntime.InferenceSession(onnx_model_path)
        if session is not None:
            print(f"addpunc ONNX model loaded from {onnx_model_path} successfully.")

        vocab = None
        current_path = os.path.dirname(os.path.abspath(__file__))
        w2i_path = os.path.join(current_path, "zhCN", "w2i.txt")
        vocab = [Vocab(w2i_path)]

    except Exception as e:
        raise Exception(f"Error loading ONNX model file {onnx_model_path}: due to {e}")

    return session, vocab


def add_punc_for_transcription(input_transcription_file, temp_output_folder, output_transcription_withpunc_file, session=None, vocab=None):
    try:
        if session is None or vocab is None:
            return None
        ...
        # current_path = os.path.dirname(os.path.abspath(__file__))
        # onnx_model_path = os.path.join(current_path, "zhCN", "punc.onnx")
        # try:
        #     session = onnxruntime.InferenceSession(onnx_model_path)
...
class auto_addpunc_for_transcription():
    def __init__(self, onnx_model_path):
        self.session, self.vocab = init_add_punc_model(onnx_model_path)


    def add_punc_for_transcription(self, input_transcription_str, temp_folder):
        try:
            if self.session is None or self.vocab is None:
                return input_transcription_str

            if input_transcription_str.strip() == "":
                return input_transcription_str
            
            if not os.path.exists(temp_folder):
                return input_transcription_str

            bpe_in_file = os.path.join(temp_folder, "bpe.in.text")
            with open(bpe_in_file, "w", encoding="utf-8") as f:
                f.write(input_transcription_str)

            if not os.path.exists(bpe_in_file):
                return input_transcription_str

            bpe_out_file = os.path.join(temp_folder, "bpe.out.text")
            BPEmain(bpe_in_file, bpe_out_file)
            if not os.path.exists(bpe_out_file):
                return input_transcription_str
            ...
            lines = [line.strip() for line in lines]
            lines = " ".join(lines).split()
            scores = get_predictions_from_ONNX(self.session, self.vocab, lines, lang_shift)
            result = tag2str(lines, scores[0].tolist())
            return result
...
if __name__ == "__main__":
    add_punc_for_transcription("bpe.in.text", "temp", "output_withpunc.txt")

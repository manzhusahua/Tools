...
def updata_context(metadta_path,output_metadata_file):
    ...
    for i in index:
        ...
        elif str(jie) != "0" and ("_".join(sid.split("_")[-3:-1])+"_"+str(int(jie)-1) in id):
            data.loc[i, 'has_left_context'] = 1
        else:
...
def updata_context_WAVNAME(metadta_path,output_metadata_file):
    data = pandasForSortingCSV.read_csv(metadta_path, sep='|', encoding='utf-8',low_memory=False)
    index = list(range(data.shape[0]))
    id = []
    for i in index:
        line = data.iloc[i]
        sid = '_'.join(line['sid'].split("_")[-2:])
        if sid not in id:
            id.append(sid)
    for i in index:
        line = data.iloc[i]
        ...
        zhang = sid.split("_")[-2]
        jie = sid.split("_")[-1]
        if str(jie) == "0" and (sid.split("_")[-2]+"_1" in id):
            data.loc[i, 'has_right_context'] = 1
        elif str(jie) == "0" and (str(int(zhang)+1).zfill(7)+"_0" in id):
            data.loc[i, 'has_right_context'] = 1
        ...
        word = str(int(zhang)-1).zfill(7)+"_"
        # chunk6624ee2cb51911ee97f5000d3ae5703a_000004_00000_0000400000_0
        # chunk421235dab51911ee97f5000d3ae5703a_000001_00000_0000400001_0
        if str(jie) == "0" and (len([x for i,x in enumerate(id) if x.find(word) != -1]) !=0):
            # print(len([x for i,x in enumerate(id) if x.find(word) != -1]))
...
if __name__ == "__main__":
    ...
    # updata_context(r"C:\Users\v-zhazhai\Downloads\metadata_Thomas_audiobook_v1.csv",r"C:\Users\v-zhazhai\Downloads\metadata_Thomas_audiobook_v1_1.csv")
